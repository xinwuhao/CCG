  ...
  <div>
    ...
    >
      ...
      <template #footer>
        ...
        <el-button type="primary" @click="getlong('ruleForm')">确 定</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
let rules = ref<any>({
  img: [{ required: true, message: "请输入用户名", trigger: "blur" }],
});
let btn = (e: any) => {
  console.log(e);
};
...
//确定按钮
let getlong = (formName: any) => {
  console.log(swiper.value);
  ref[formName].validate((valid: any) => {
    if (valid) {
      alert("submit!");
    } else {
