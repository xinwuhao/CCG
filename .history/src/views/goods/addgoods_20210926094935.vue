    ...
    <div v-if="judge == 0">
      ...
      <div style="margin-top: 20px">
        <el-table :data="tableData" border style="width: 100%">
          ...
          <el-table-column prop="presentPrice" label="商品原价"> </el-table-column>
          <el-table-column prop="originalPrice" label="商品现价"> </el-table-column>
    ...
    <div v-if="judge !== 0">
      <el-tabs v-model="judge">
        ...
        <el-tab-pane label="商品规格" name="3" disabled>
          ...
          <div>
            <el-button @click="judge = '2'">取消</el-button>
            <el-button type="primary" @click="confirm2">确定</el-button>
          </div>
        </el-tab-pane>
        <el-tab-pane label="商品详情" name="4" disabled>
...
let handleRemove = (response: any, file: any, fileList: any) => {
...
let confirm3 = () => {
  api
    .addGoods({
      name: swiper.value.name, // 商品名称
      category: swiper.value.category, // 商品分类
      cover: swiper.value.cover, // 封面图
      pic: list.value, // 商品图片
      isHot: swiper.value.isHot, // 是否热销
      video: [], // 商品视频
      isRecommend: swiper.value.isRecommend, // 是否推荐
      originalPrice: swiper.value.originalPrice, // 原价
      presentPrice: swiper.value.presentPrice, // 现价
      discount: [], // 优惠 传空
      detail: discount.value, // 商品详情
      spec: user.value, // 商品规格
      introduction: swiper.value.introduction, // 商品简介
      company: swiper.value.company, // 商品单位
      stock: swiper.value.stock, // 商品库存
      isNewGood: swiper.value.isNewGood, // 是否新品
      comment: [], // 商品评价
      isShow: true, // 是否显示
      sellDesc: swiper.value.sellDesc, // 推荐介绍
      productionDesc: [], // 产品介绍
    })
    .then((res: any) => {
      console.log(res);
