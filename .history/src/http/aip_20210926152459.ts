...
export default {
  ...
  // 修改商品参数
  updateParams({ id, url, name, desc, parentId }: { parentId: any, id: string, url: string, name: string, desc: string }) {
  ...
  // 修改秒杀
  updateSeckill({ id, start_time, end_time, goods_number, price, goods, isShow }: { id: any, price: string, start_time: string, end_time: string, goods_number: string, goods: any, isShow: boolean }) {
    return http.post('/admin/updateSeckill', {
      id, start_time, end_time, goods_number, price, goods, isShow, id
    })
  },
  // 获取优惠券
  ...
  updateCoupon({ id, name, amount, threshold, start_time, end_time, number, isShow }: { id: any, name: string, amount: string, threshold: string, start_time: string, end_time: string, number: string, isShow: boolean }) {
